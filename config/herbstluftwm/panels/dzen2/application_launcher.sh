...
app_bar(){
        ...
        echo "^fn(Noto Sans Display Nerd Font-72)^ca(1, herbstclient spawn alacritty -e nvim)^fg($green)^ca()"
        ...
        echo "^fn(Noto Sans Display Nerd Font-72)^ca(1, herbstclient spawn slack)^fg($red)^ca()"
                ...
                app_bar | dzen2  -bg "$normal_bg" -fg "#240e28" -ta c -h 100 -x 650 -w 650 -y 900 -m h -l 6 -sa c -p -e "ungrabkeys" -title-name app_bar &
